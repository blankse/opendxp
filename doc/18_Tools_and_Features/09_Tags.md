...
Using tags, you can easily filter OpenDXP Elements (Documents, Data Objects, Assets) and keep the additional search criteria to the application.

## Tags Definition

The available tags have to be defined centrally (user permission *tags configuration* is necessary).
 
![Tags OpenDXP menu](../img/tags_menu.png)
...
For accessing and working with tags via API, take a look into `OpenDxp\Model\Element\Tag`.
...
```php
$tags = \OpenDxp\Model\Element\Tag::getTagsForElement('asset', 30);
dump($tags);
```

The output will be an array with the `OpenDxp\Model\Element\Tag` elements.

```
array:3 [▼
  0 => OpenDxp\Model\Element\Tag {#7351 ▼
    #id: 9
    #name: "Italy"
    #parentId: 7
    #idPath: "/7/"
    #children: null
    #parent: OpenDxp\Model\Element\Tag {#7354 ▶}
    #dao: OpenDxp\Model\Element\Tag\Dao {#7349 ▶}
    id: 9
    name: "Italy"
    parentId: 7
    idPath: "/7/"
  ...
  1 => OpenDxp\Model\Element\Tag {#7357 ▶}
  2 => OpenDxp\Model\Element\Tag {#7345 ▶}
]  
```

#### *Assign a New Tag to an Element*

To add a new tag to an element, you first need to create the new tag. Then you just have to use the `\OpenDxp\Model\Element\Tag::addTagToElement` method to assign the tag to the chosen element (by element id).

```php
$tag =  new \OpenDxp\Model\Element\Tag();
try {
    $tag->setName('newtag')->save();
    \OpenDxp\Model\Element\Tag::addTagToElement('asset', 30, $tag);
} catch (Exception $e) {
// ....
}
