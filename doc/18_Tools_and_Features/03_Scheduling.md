...
## General
Every element type in OpenDXP (documents, objects, assets) has implemented a scheduler which provides the ability to create tasks like:
...
In the OpenDXP backend UI find the icon: 
...
When the `./bin/console opendxp:maintenance -j scheduledtasks` script will finish processing jobs you could see the changes.
