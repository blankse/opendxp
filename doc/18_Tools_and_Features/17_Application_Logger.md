...
To use this feature, please enable the `OpenDxpApplicationLoggerBundle` in your `bundle.php` file and install it accordingly with the following command:

`bin/console opendxp:bundle:install OpenDxpApplicationLoggerBundle`

:::

## General

The application logger bundle is a tool which developers can use to log certain events and errors within a OpenDXP powered application.

<div class="inline-imgs">

The logs are visible and searchable within the OpenDXP backend GUI ![Tools menu](../img/Icon_tools.png) **Tools -> Application Logger**:
...
The application logger is a PSR-3 compatible component and available on the service container as service `OpenDxp\Bundle\ApplicationLoggerBundle\ApplicationLogger`
...
App\YourService: 
    calls:
        - [setLogger, ['@OpenDxp\Bundle\ApplicationLoggerBundle\ApplicationLogger']]
```
...
and make full use of monolog's possibilities. To do so, OpenDXP provides the `ApplicationLoggerDb` monolog handler which
...
monolog:
    handlers:
        ...
        application_logger_db:
            type: service
            id: OpenDxp\Bundle\ApplicationLoggerBundle\Handler\ApplicationLoggerDb
...
Adds a console logger and sets the minimum logging level to *INFO* (overwrites log level in OpenDXP system settings):

```php
$logger = \OpenDxp\Bundle\ApplicationLoggerBundle\ApplicationLogger::getInstance("SAP_exporter", true); 
// returns a PSR-3 compatible logger, registers a custom app logger as `opendxp.app_logger.SAP_exporter` on the service container
