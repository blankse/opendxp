...
```php
$new = OpenDxp\Model\Element\Service::cloneMe($source)
```
...
```php
$user = \OpenDxp\Model\User::getById(123);

$assetService = new \OpenDxp\Model\Asset\Service($user);
$assetService->copyAsChild($target, $source);

$documentService = new \OpenDxp\Model\Document\Service($user);
$documentService->copyAsChild($target, $source); // additional arguments are available for inheritance, ...

$objectService = new \OpenDxp\Model\DataObject\Service($user);
