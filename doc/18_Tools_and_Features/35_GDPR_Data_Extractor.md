...
export data that is stored for a specific person in different data sources like `data objects`, `sent mails`, `OpenDXP backend 
user`, etc.
...
```yml
# Default configuration for "OpenDxpAdminBundle"
opendxp_admin:
    gdpr_data_extractor:
...
OpenDXP ships with a reasonable default configuration. By using it, all data object classes are considered in the search, 
...
It is possible to attach additional data sources to the GDPR Data Extractor with OpenDXP Bundles. Thereby specific data 
...
3) Register your custom implementation as service. The service needs to be tagged with the tag `opendxp.gdpr.data-provider`.
   ...
   your service definition:
    ...
    services:
        ...
        App\GDPR\DataProvider\MyCustomDataProvider:
            tags:
                - { name: opendxp.gdpr.data-provider }
    ```

For an example see the implementation for the [customers data provider](https://github.com/open-dxp/customer-data-framework/blob/master/src/GDPR/DataProvider/Customers.php) 
