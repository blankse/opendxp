...
You can use the OpenDxpWebToPrintBundle functionality to accomplish this.

Please make sure that you have set up and installed the OpenDxpWebToPrintBundle correctly ("Settings" -> "Web2Print Settings").

You need to enable and install the OpenDxpWebToPrintBundle via the bundles.php, and then you 
...
class BlogController extends FrontendController
{
    public function indexAction(Request $request): Response
    {
        ...
        return new Response(
            \OpenDxp\Bundle\WebToPrintBundle\Processor::getInstance()->getPdfFromString($html),
            200,
            array(
...
class BlogController extends FrontendController
{
    public function indexAction(Request $request): Response
    {
        ...
        $adapter = \OpenDxp\Bundle\WebToPrintBundle\Processor::getInstance();
        //add custom settings if necessary
        if ($adapter instanceof \OpenDxp\Bundle\WebToPrintBundle\Processor\PdfReactor) {
        ...
        } elseif ($adapter instanceof \OpenDxp\Bundle\WebToPrintBundle\Processor\Gotenberg) {
            $params = Config::getWeb2PrintConfig();
            $params = json_decode($params['gotenbergSettings'], true) ?: [];
        } elseif ($adapter instanceof \OpenDxp\Bundle\WebToPrintBundle\Processor\Chromium) {
            $params = Config::getWeb2PrintConfig();
