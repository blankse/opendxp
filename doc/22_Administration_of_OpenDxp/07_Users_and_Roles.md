...
User permissions in OpenDXP are based on a users and roles concept. Each user can have several roles and both - users and roles - can have permissions. 
 
Users and roles are configured in OpenDXP backend UI at `Settings` > `Users & Roles` > `Users` and
...
## Permissions
In OpenDXP, there are two levels of user permissions: 
1) Permissions on system components,
2) Permissions on data elements (Assets, Data Objects, and Documents). 

Permissions can be granted to individual users or groups of users (called "roles" in OpenDXP). 
...
* **Clear Cache**: defines if a user may clear OpenDXP cache (internal cache and response cache if configured)
...
By using the event `OpenDxp\Event\ElementEvents::ELEMENT_PERMISSION_IS_ALLOWED`, it is possible to dynamically manipulate a user's permissions on a specific element on request.
