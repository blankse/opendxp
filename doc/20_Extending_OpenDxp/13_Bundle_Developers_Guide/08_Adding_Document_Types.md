...
Prior to 10.6, the only way to define custom documents was to create them in a special namespace `OpenDxp\Model\Document`. This
...
The document **must** extend `OpenDxp\Model\Document`. Lets create a `Book` document (the namespace does not matter
...
class Book extends \OpenDxp\Model\Document
{
...
Next we need to update `opendxp.documents.type_definitions.map` configuration to include our document. This can be done in any config
...
If you want to override an existing class please use the `opendxp:models:class_overrides` instead.
