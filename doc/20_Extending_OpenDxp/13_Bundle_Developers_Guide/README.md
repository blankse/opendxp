...
Since OpenDXP utilizes the powerful Symfony Bundle system, let us refer to the [Symfony Bundle Documentation](https://symfony.com/doc/current/bundles.html) on how to get started with your custom bundles. A bundle can do anything - in fact, core OpenDXP functionalities like the admin interface are implemented as bundles. From within your bundle, you have all possibilities to extend the system, from defining new services or routes to hook into the event system or provide controllers and views.
...
There is a special kind of bundle implementing `OpenDxp\Extension\Bundle\OpenDxpBundleInterface` which gives you additional
possibilities. These bundles provide a similar API as plugins did in previous versions:

* The bundle shows up in the `opendxp:bundle:list` command with info if the bundle can be installed or uninstalled.
...
With the [OpenDXP Bundle Generator](https://github.com/open-dxp/bundle-generator), we provide a tool for generating bundle skeletons. You can install and activate this bundle in your development instance and simplify starting the OpenDXP bundle development.  

```
# generate bundle interactively
$ bin/console opendxp:generate:bundle

# generate bundle with a given name and don't ask questions
$ bin/console opendxp:generate:bundle --namespace=Acme/FooBundle --no-interaction
...
Then only translations stored in a dedicated domain table e.g. `translations_DOMAIN` are used by the OpenDXP translation service.
...
To hook into core functions, you can attach to any event provided by the [OpenDXP event manager](../11_Event_API_and_Event_Manager.md).
...
* [OpenDXP Event Manager](../11_Event_API_and_Event_Manager.md) for a list of available events.
...
If you need persistent storage, create a unique directory in `OPENDXP_PRIVATE_VAR`, e.g. `var/bundles/YourBundleName`.
