...
The `BundleCollection` is a container which is used to register every used bundle. As OpenDXP gathers bundles from multiple 
...
While Symfony's standard edition uses a `registerBundles` method building an array of bundles to load, OpenDXP expects you
...
load the bundles your bundle depends on in their `App\Kernel` or you can implement the [`DependentBundleInterface`](https://github.com/open-dxp/opendxp/blob/1.x/lib/HttpKernel/Bundle/DependentBundleInterface.php)
