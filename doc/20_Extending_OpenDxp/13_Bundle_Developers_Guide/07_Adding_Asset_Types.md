...
The asset **must** extend `OpenDxp\Model\Asset`. Let´s create a class for InDesign (the namespace does not matter)
...
[github](https://github.com/open-dxp/opendxp/tree/11.x/models/Asset).
...
class InDesign extends \OpenDxp\Model\Asset
{
...
It needs to extend `opendxp.asset.asset`, be located in the namespace `opendxp.asset` and named after the
...
Next we need to update the `opendxp.assets.type_definitions.map` configuration to include our asset. This can be done in any config
