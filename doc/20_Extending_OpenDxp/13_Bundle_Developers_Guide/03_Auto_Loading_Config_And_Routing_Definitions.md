...
in `config/` (optionally importing config files from bundles or other locations). OpenDXP extends the config loading
by trying to automatically load `config.yaml` and `routing.yaml` (`php`, and `xml` types are also supported) from every active
bundle if they are defined in the directory `config/opendxp`. By making use of this auto-loading mechanism,
you can provide routing and config from within your bundle.

When loading the configuration, OpenDXP will first try to load an environment specific config file (e.g. `config_dev.yaml`)
...
From the `OpenDxp\Config\BundleConfigLocator`:
