...
The OpenDXP workflow management can also be used via PHP API. 

## Using Workflow API to modify Places of Elements

Since the OpenDXP workflow management is based on the Symfony workflow component, also its API can be 
used the same way. For details see [Symfony docs](https://symfony.com/doc/current/workflow/usage.html).

Additionally OpenDXP provides the `\OpenDxp\Workflow\Manager`, which provides additional functionality for working
...
/**
 * $object ... your element, e.g. a OpenDXP data object
...
if($workflow->can($object, 'publish')) {
    ...
    //modify workflow with OpenDXP Workflow Manager - notes are written with additional data
    $additionalData = [
    ...
    /**
     * $workflowManager OpenDxp\Workflow\Manager from Symfony container
     */
...
In addition to the Symfony events, OpenDXP provides two additional events for global actions: 
- `opendxp.workflow.preGlobalAction`
- `opendxp.workflow.postGlobalAction`
See [WorkflowEvents](https://github.com/open-dxp/opendxp/blob/1.x/lib/Event/WorkflowEvents.php) for details. 
