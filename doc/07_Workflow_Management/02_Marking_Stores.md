...
The OpenDXP workflow engine provides several different ways how to store the actual places of a subject. These are 
...
objects a select field (or maybe input field) would be the right OpenDXP field to store the places when the single_state 
...
Works similar to single_state and data_object_multiple_state but is able to store different places in different OpenDXP data object 
...
to automatically provide the correct select options. Just setup `@opendxp.workflow.place-options-provider` as options 
