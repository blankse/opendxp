...
The workflow is available only for instances of `OpenDxp\Model\DataObject\Product` objects. We use the workflow 
...
to OpenDXP as empty objects only with SKU.

I need to have ability to decide which products would be used in OpenDXP, the rest of products I want to reject.
...
(...)
    transitions:
        ...
        publish:
            ...
            to: accepted
            guard: "is_fully_authenticated() and is_granted('ROLE_OPENDXP_ADMIN')"
            options:
