...
## General
OpenDXP Workflow Management provides configuration of multiple workflows on OpenDXP elements (assets, documents, data 
...
with a few OpenDXP specific features. So, before using OpenDXP Workflow Management it makes sense to know 
the basics of Symfony workflow component.


### Concepts 
**Workflow** 
A workflow is a model of a process an element in OpenDXP goes through. It consists of *places*, *transitions*, a 
...
**Marking Store**
The marking store stores the current place(s) for each element. OpenDXP ships with a couple of stores that can be configured
...
## Configuration
The workflow configuration takes place in the Symfony configuration tree in the OpenDXP namespace. For details of 
configuration options see inline comments and documentation (call command `bin/console config:dump-reference OpenDxpCoreBundle`)
or [Configuration Details](./01_Configuration_Details/README.md).


## Events

The OpenDXP workflow management fires several events that can be used to customize and extend functionality. For details
see [Working with PHP API](./09_Working_with_PHP_API.md).


## User Notifications
Notifications (via email or OpenDXP notifications) can be configured to be sent to users when an transition takes place. 
            ...
            options:
                notificationSettings:
                    - 
                      ...
                      # Type of mail source. 
                      mailType: 'template' # this is the default value, One of "template"; "opendxp_document"
                      
                      # Path to mail source - either Symfony path to template or fullpath to OpenDXP document. 
                      # Optional use %%_locale%% as placeholder for language.
                      mailPath: '@OpenDxpCore/Workflow/NotificationEmail/notificationEmail.html.twig' #this is the value
...
```
...
- Overwrite the template `@OpenDxpCore/Workflow/NotificationEmail/notificationEmail.html.twig` or configure your own 
  ...
  the service `OpenDxp\Workflow\Notification\NotificationEmailService`.

- Configure a OpenDXP Mail Document and use full power of OpenDXP Mail Documents, with Controller, Action, Placeholders, 
...
their current places and a workflow graph is added to OpenDXP element detail page. 
