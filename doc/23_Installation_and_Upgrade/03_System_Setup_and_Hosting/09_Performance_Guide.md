...
As OpenDXP is based on Symfony framework for backend requests processing, it is highly recommended 
...
Note: `memory_limit` should be set to at least `200M` when using preloading with OpenDXP.
...
OpenDXP uses Data Access Object or DAO pattern to isolate the business logic from low level data access operations. 
These data access operations play a vital role in performance of OpenDXP application features 
...
### OpenDXP Caching (Redis)
OpenDXP uses extensively caches for different types of data. 
The primary cache is a pure object cache where every element (document, asset, object) in OpenDXP is cached 
as it is (serialized objects).

This Primary Cache is utilizing the `OpenDxp\Cache` interface to store the objects. 
`OpenDxp\Cache` utilizes a `OpenDxp\Cache\Core\CoreCacheHandler` to apply OpenDXP's 
caching logic on top of a PSR-6 cache implementation which needs to implement cache tagging. 
OpenDXP uses the `opendxp.cache.pool` Symfony cache pool, you can configure it according to your needs, 
but it's crucial that the pool supports tags.

By default, OpenDXP uses the Doctrine connection and write to your DB's `cache_items` tables. 
...
from Symfony, however, OpenDXP recommends to use `Redis` adapter for a performance boost with optional clustering 
and fail-over support.

Configure `Redis` adapter for `OpenDxp\Cache` using these settings:
```yaml
...
Beware that element data gets added to the cache on first request of the data. To warm-up the cache after clearing the cache (e.g. caused by a deployment), execute `bin/console opendxp:cache:warming` so users experience optimal performance even on first access of an element.
...
### OpenDXP Full Page Cache
OpenDXP full page cache enables you to cache the content returned by a controller action. 
...
Only use sessions when really necessary. The OpenDXP full-page cache detects the usage of sessions in the code 
and disables itself if necessary.

### Static-Page-Generator
OpenDXP offers a static page generator service, which generates static HTML files out of pages (documents). 
...
### In-Template Caching
OpenDXP provides a Twig extension `opendxp_cache` for in-template caching, which is used to cache some parts 
...
```twig
{% set cache = opendxp_cache("main_navigation_cache", 60) %}
{% if not cache.start() %}
    {% set navStartNode = document.getProperty('navigation_root') %}

    {% if not navStartNode is instanceof('\\OpenDxp\\Model\\Document') %}
...
Read more about `opendxp_cache` [here](../../02_MVC/02_Template/02_Template_Extensions/README.md).
...
> Note: OpenDXP sends the right headers for Varnish if full-page cache is enabled
