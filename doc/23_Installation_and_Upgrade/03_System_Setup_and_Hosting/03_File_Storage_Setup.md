# File Storage Setup
OpenDXP uses a powerful & flexible file storage library, called [Flysystem](https://github.com/thephpleague/flysystem) 
...
### Storage Migration
If you are switching to different a storage type, it is often required to migrate contents from old storage to the newly configured one. OpenDXP provides a command to solve the purpose of migrating contents from between storages, which in turn uses the flysystem `listcontents` API to read contents recursively from old (source) storage and copy contents to the new (target) storage. 

Follow these steps to migrate the content:
1. Create a Flysystem configuration for source & target storages. It is important to follow the naming structure, as follows: source config node: `opendxp.{storagetype}.storage.source` & target config node: `opendxp.{storagetype}.storage.target`

Few examples,

   | Migration Task | source node                      | target node                      |
   |----------------|----------------------------------|----------------------------------|
   | `asset`        | opendxp.asset.storage.source     | opendxp.asset.storage.target     |
   | `thumbnail`    | opendxp.thumbnail.storage.source | opendxp.thumbnail.storage.target |
   | `version`      | opendxp.version.storage.source   | opendxp.version.storage.target   |
   
```yaml
flysystem:
    storages:
        opendxp.asset.storage.source:
            adapter: 'local'
            visibility: public
            options:
                directory: '%kernel.project_dir%/public/var/assets'

        opendxp.asset.storage.target:
            adapter: 'aws'
            visibility: public
            options:
                client: 'assets_s3'
        ...
        opendxp.thumbnail.storage.source:
            adapter: 'local'
            visibility: private
            directory_visibility: public
            options:
        ...
        opendxp.thumbnail.storage.target:
            adapter: 'aws'
            visibility: public
            options:
                client: 'assets_s3'
...
2. Run command `opendxp:migrate:storage` with storage type argument: 
   `bin/console opendxp:migrate:storage asset`
