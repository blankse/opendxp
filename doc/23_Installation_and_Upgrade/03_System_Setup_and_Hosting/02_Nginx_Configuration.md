...
- OpenDXP was installed into: `/var/www/opendxp`; therefore, the Document-Root is: `/var/www/opendxp/public`
- Logfiles are written to the default location `/var/log/nginx`. If you prefer to have the logs together with the OpenDXP Logs: these are in `/var/www/opendxp/var/log`.
- PHP-FPM is configured to listen on the Socket `/var/run/php/opendxp.sock`. If your setup differs, change the `server` directive within the `upstream` block accordingly.
...
upstream php-opendxp {
    server unix:/var/run/php/opendxp.sock;
}
...
server {
    ...
    # OpenDXP Head-Link Cache-Busting
    ...
    # or redirect these suffixes directly to your CDN URL. Additionally you should configure the frontend url prefixes accordingly, see: docs/Development_Documentation/Installation_and_Upgrade/System_Setup_and_Hosting/File_Storage_Setup.html
...
```nginx
# Zone to Limit OpenDXP On-demand Image generation
...
```nginx
    # OpenDXP On-Demand Thumbnail generation
    # with Rate-Limit.
    location ~* .*/(image|video)-thumb__\d+__.* {
