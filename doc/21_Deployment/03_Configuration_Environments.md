...
OpenDXP supports different configurations for different environments (dev, test, stage, prod, ...) as well as custom 
configurations including a fallback mechanism. 

OpenDXP is relying on Symfony's environments, with some extras, however all the essential 
...
OpenDXP provides the possibility to configure them using various storage types. 
Available storages are (in priority order): 
- Symfony Config (YAML, needs container rebuild)
- OpenDXP [`SettingsStore`](../19_Development_Tools_and_Details/42_Settings_Store.md)
...
The default storage directory for Symfony Config files is defined by `OPENDXP_CONFIGURATION_DIRECTORY`.
...
For example, to revalidate image or video thumbnails either run command `opendxp:thumbnails:clear` or call `Asset\Image\Thumbnail\Config::clearTempFiles()` after deploying changes on thumbnail configurations.
