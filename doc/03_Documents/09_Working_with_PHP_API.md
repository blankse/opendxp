...
OpenDXP provides the object orientated PHP API to work with Documents.
...
> Type of the document (page, folder, link, email, snippet etc.) The complete list can be obtained using: `\OpenDxp\Model\Document::getTypes()`
> Every type has its own model, for example: a link document could be created by `new \OpenDxp\Model\Document\Link`.

The example below, shows how to create simple page document via the PHP API. 

```php
//CREATE A PAGE DOCUMENT
$page = new \OpenDxp\Model\Document\Page();
...
If you'd like to get a document you can use the `getById()` or `getByPath()` method from the `\OpenDxp\Model\Document` class.

Find below, the list of available methods for loading documents.

| Reference                          | Arguments    | 
|------------------------------------|--------------|
| \OpenDxp\Model\Document::getById   | int $id      |
| \OpenDxp\Model\Document::getByPath | string $path |

The following code presents how to get the wysiwyg editable value of the document.

```php
//LOAD A DOCUMENT
$document = \OpenDxp\Model\Document::getById(4);
if($document) {
    //the logic when the type of the document is page
    /** @var \OpenDxp\Model\Document\Page $document */

    /** @var \OpenDxp\Model\Document\Editable\Wysiwyg $wysiwygElement */
...
Just load the document and use `delete` method available in `\OpenDxp\Model\Document`.

```php
$document = \OpenDxp\Model\Document::getById(110);
if ($document) {
    $document->delete();
}
```
...
```php
$listing = new \OpenDxp\Model\Document\Listing(); 
...
/** @var \OpenDxp\Model\Document\Listing $listing */
$listing = \OpenDxp\Model\Document::getList([
    'unpublished' => true,
