...
|--------------------------|----------------------|---------------------------------------|-------------------------------------------------------------------------|
| `getThumbnail($name)`    | (string/array) $name | OpenDxp\Model\Asset\Image\Thumbnail   | Get a specific thumbnail of the image                                   |
...
| `getImage()`             | -                    | \OpenDxp\Model\Asset\Image            | The asset object which is referenced (`\OpenDxp\Model\Asset\Image`)     |
...
{% else %}
    {% set thumbnail = opendxp_image("myImage").getThumbnail("myThumbnail") %}
...
<div>
    <p>
        ...
        {% if not editmode %}
            {% set image = opendxp_image("myImage") %}
...
array(1) {
  [0] => array(6) {
    ...
    ["data"] => array(2) {
      [0] => object(OpenDxp\Model\Element\Data\MarkerHotspotItem)#171 (3) {
        ["name"] => string(30) "checkbox_data_added_to_hotspot"
        ["type"] => string(8) "checkbox"
        ["value"] => bool(true)
      }
      [1] => object(OpenDxp\Model\Element\Data\MarkerHotspotItem)#172 (3) {
...
array(1) {
  [0] => array(4) {
    ...
    ["left"] => float(69.9)
    ["data"] => array(1) {
      [0] => object(OpenDxp\Model\Element\Data\MarkerHotspotItem)#173 (3) {
