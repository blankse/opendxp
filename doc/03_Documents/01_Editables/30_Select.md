...
{% else %}
    <p>
        {{ "Something is valid for" | trans }}:{{ opendxp_select("valid_for").getData() | trans  }}
    </p>
{% endif %}
```
...
```twig
{% if editmode %}
    {% if opendxp_select('valid_for').isEmpty() %}
        {% do opendxp_select('valid_for').setDataFromResource('unlimited') %}
    {% endif %}
