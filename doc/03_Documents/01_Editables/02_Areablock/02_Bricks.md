...
A brick is an instance of `OpenDxp\Extension\Document\Areabrick\AreabrickInterface` which can either be auto-loaded
...
existing templating extensions and [OpenDXP editables](../README.md).
...
A basic brick implementation could look like the following. As it is defined in the special namespace, OpenDXP will
...
tag `opendxp.area.brick`. Bricks defined manually will be excluded from the auto-registration, even if they're
...
services:
    App\Document\Areabrick\Iframe:
        arguments: ['@logger']
        tags:
            - { name: opendxp.area.brick, id: iframe }
```
...
For convenience, you can create a new brick by extending `OpenDxp\Extension\Document\Areabrick\AbstractTemplateAreabrick`
...
You can optionally implement `OpenDxp\Extension\Document\Areabrick\PreviewAwareInterface` to add a custom html tooltip
...
{% set urlField = opendxp_input("iframe_url") %}
{% set widthField = opendxp_numeric("iframe_width") %}
{% set heightField = opendxp_numeric("iframe_height") %}
{% set transparentField = opendxp_checkbox("iframe_transparent") %}
