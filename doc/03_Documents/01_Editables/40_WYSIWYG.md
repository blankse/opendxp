...
## Add a Custom Editor
Make sure that you add the Editor to `opendxp.wysiwyg.editors`. This array can be used to have different editors for different use cases (documents, objects ...):
```javascript
if(!parent.opendxp.wysiwyg) {
    parent.opendxp.wysiwyg = {};
    parent.opendxp.wysiwyg.editors = [];
}
parent.opendxp.wysiwyg.editors.push("Custom_Editor");
```

The Editor als needs to dispatch the `opendxp.events.changeWysiwyg` to set the value from the WYSIWYG-Field in the core.
```javascript
