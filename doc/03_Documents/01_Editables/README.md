...
In some cases, especially with areablocks, editables could throw exceptions. Since OpenDXP internally uses the `__toString` method to render
the editables, OpenDXP can't throw exceptions there. Therefore OpenDXP catches the exception, and puts it out as string if DEBUG Mode is enabled.
...
| [Relation](./12_Relation_Many-To-One.md)                           | Provides possibility to create a reference to any other element in OpenDXP (document, asset, object).                                                        |
| [Relations (Many-To-Many Relation)](./13_Relations_Many-To-Many.md) | Provides possibility to edit multiple references to other elements in OpenDXP (documents, assets, objects).                                                  |
...
| [Renderlet](./28_Renderlet.md)                                     | The renderlet is a special container which is able to handle every object in OpenDXP (Documents, Assets, Objects).                                           |
