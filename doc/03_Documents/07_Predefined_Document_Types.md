...
OpenDXP provides the ability to define preconfigurations for documents, which makes it easier for editors to work with 
...
`printcontainer` or `printpage` are only available if the OpenDxpWebToPrintBundle is enabled and installed.  
`newsletter` is only available if the OpenDxpNewsletterBundle is enabled and installed.
