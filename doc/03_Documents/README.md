# Documents
Documents are the CMS part of OpenDXP and are the way to go for managing unstructured contents using pages, content snippets and navigations. 

## Document Types
OpenDXP offers different types of documents and each of them offers functionality specific for the intended use-case. 
...
OpenDXP is shipped with a default controller containing a default action, which is called when only a template is given to the document.
...
* Documents follow the MVC pattern; therefore, OpenDXP requires that there is at least one controller with an action and a template file.
* OpenDXP comes with a DefaultController containing a defaultAction and a template file.
* Because of the MVC architecture, there is a clear separation between your data, the functionality and the templates.
* Templates for OpenDXP are created with [Twig](https://twig.symfony.com/).
...
```bash
bin/console opendxp:documents:cleanup <type1> <type2> <type3>
```
