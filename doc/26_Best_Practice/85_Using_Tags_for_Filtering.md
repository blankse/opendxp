# Using OpenDXP Tags for Filtering in Frontend

The [OpenDXP Tags](../18_Tools_and_Features/09_Tags.md) functionality is primarily designed
 as OpenDXP Backend UI functionality for tagging and filtering elements. Nevertheless sometimes it can be quite useful to implement custom frontend filtering functionality based on the OpenDXP
 Tags. 
...
$tagList = new \OpenDxp\Model\Element\Tag\Listing();
...
*/
protected function convertTagToArray(\OpenDxp\Model\Element\Tag $tag, $assignedTagIds)
{
    $tagArray = [
        "id" => $tag->getId(),
        "text" => $tag->getName()
        ...
        if($values)
        {
            ...
            foreach ($values as $tagId) {
                ...
                if ($request->query->getBool("considerChildTags")) {
                    $tag = \OpenDxp\Model\Element\Tag::getById((int)$tagId);
                    if ($tag) {
