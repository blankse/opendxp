...
Sometimes it might be useful to add additional buttons to the object editor (or any other editor) in OpenDXP Backend 
...
1) Create a bundle with a OpenDXP Backend Interface java script extension as described 
...
document.addEventListener(opendxp.events.postOpenObject, (e) => {
    if (e.detail.object.data.general.className === 'ShopProduct') {
        e.detail.object.toolbar.add({
            text: t('show-pdf'),
            iconCls: 'opendxp_icon_pdf',
            scale: 'small',
