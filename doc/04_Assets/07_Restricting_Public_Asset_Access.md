...
OpenDXP has following default behavior in terms of asset delivery: 

> All Data that is stored as OpenDXP Assets can be accessed via its URL (e.g. https://mydomain.com/my-assetfolder/my-asset.jpg) 
> and therefore is public available without login or other access restriction!
> 
> As a consequence confidential data **must not** be stored as OpenDXP Assets without additional protection measures.
...
If further restriction is needed, OpenDXP provides two options for doing so: 


### Option 1 - Restricting access to certain assets completely  

All confidential assets need to be stored within one (or a few) folder(s), e.g. within `/protected` (to set up OpenDXP 
...
server anymore. As a consequence also using the direct link for downloading or using the OpenDXP generated img tags for 
...
use OpenDxp\Model\Asset\Service;
