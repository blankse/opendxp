...
and have a look at the tables created by OpenDXP.
...
Nevertheless, you can use inheritance for field collections for data maintenance by [overriding the class](../../../20_Extending_OpenDxp/03_Overriding_Models.md) which contains the field collection field and adding a custom getter method to this overriding class:
```php
...
public function getFieldCollection(): mixed
{
    ...
    $inheritedData = DataObject\Service::useInheritedValues(true, function() use ($data) {
        if (\OpenDxp\Model\DataObject::doGetInheritedValues($this) && $this->getClass()->getFieldDefinition("fieldCollection")->isEmpty($data)) {
            try {
                return $this->getValueFromParent("fieldCollection");
            } catch (\OpenDxp\Model\DataObject\Exception\InheritanceParentNotFoundException $e) {
...
There could also be some UI quirks in the OpenDXP backend.
