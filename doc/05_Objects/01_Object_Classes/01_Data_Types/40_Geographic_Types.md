...
There are different geographic data types available in opendxp: `Geopoint`, `Geobounds`, `Geopolygon` and `Geopolyline`. 
...
The geopoint consists of two coordinates: latitude and longitude. In the OpenDXP GUI there is the same geopoint selector 
...
represented by `OpenDxp\Model\DataObject\Data\Geopoint`. To set a geopoint programmatically, a new 
`OpenDxp\Model\DataObject\Data\GeoCoordinates` has to be instantiated:

```php
$longitude = 107.6191228;
$latitude = -6.9174639;
$point = new \OpenDxp\Model\DataObject\Data\GeoCoordinates($latitude, $longitude);
...
form of a serialized array of `OpenDxp\Model\DataObject\Data\GeoCoordinates`. To set geopolygon data programmatically, an 
...
`OpenDxp\Model\DataObject\Data\GeoCoordinates`. To set geopolygon data programmatically, an array of Geopoints has to be 
