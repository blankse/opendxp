...
Many-To-One, Many-To-Many and Many-To-Many Objects are pure relation data types, which means they represent a relation to an other OpenDXP 
element (document, asset, object). The Many-To-One and Many-To-Many data types can store relations to any other OpenDXP element. 
...
```php
$relationId = 162;
$list = new \OpenDxp\Model\DataObject\Example\Listing();
...
In order to set a Many-To-One data field, a single OpenDXP element needs to be passed to the setter. With Many-To-Many 
...
There are several object data types which represent a relation to an other OpenDXP element. The pure relation types are
...
tab and OpenDXP issues a warning when deleting an element which has dependencies.
