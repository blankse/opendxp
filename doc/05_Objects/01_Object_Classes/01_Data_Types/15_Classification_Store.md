...
$heightValue = new \OpenDxp\Model\DataObject\Data\QuantityValue(13, 1);
...
```php
/** @var \OpenDxp\Model\DataObject\Classificationstore $classificationStore */
...
foreach ($classificationStore->getGroups() as $group) {
    ...
    foreach ($group->getKeys() as $key) {
        ...
        $value = $key->getValue();
        if ($value instanceof \OpenDxp\Model\DataObject\Data\QuantityValue) {
            $value = (string)$value;
        }
...
```php
/** @var \OpenDxp\Model\DataObject\Classificationstore\Key $key */
...
$definition = new \OpenDxp\Model\DataObject\ClassDefinition\Data\QuantityValue();
$definition->setName("height");
$definition->setTitle("Height");

$keyConfig = new \OpenDxp\Model\DataObject\Classificationstore\KeyConfig();
...
// Group
$groupConfig = new \OpenDxp\Model\DataObject\Classificationstore\GroupConfig();
...
// Collection
$collectionConfig = new \OpenDxp\Model\DataObject\Classificationstore\CollectionConfig();
...
// Add a key to group
$keyRel = new \OpenDxp\Model\DataObject\Classificationstore\KeyGroupRelation();
$keyRel->setGroupId($groupConfig->getId());
$keyRel->setKeyId($keyConfig->getId());
$keyRel->save();

// Add a group to a collection
$groupRel = new \OpenDxp\Model\DataObject\Classificationstore\CollectionGroupRelation();
