...
An image field is stored in an INT column in the database. It holds the ID of the referenced `\OpenDxp\Model\Asset\Image`. 
...
To set an object's image field programmatically, an `\OpenDxp\Model\Asset\Image` must be passed to the according setter.
...
```twig
{% if object.myImage is instanceof('OpenDxp\Model\Asset\Image') %}
    {{ object.myImage.thumbnail('myThumbnailDefinitionName').html }}
{% endif %}
```

Since `$object->getImage()` just returns an asset object, you can of course use all other thumbnail features of `OpenDxp\Model\Asset\Image`.
...
```twig
{% if object.getExternalImage() is instanceof('\\OpenDxp\\Model\\DataObject\\Data\\ExternalImage') %}
...
foreach($galleryData as $img){
   ...
   $advancedImage = new \OpenDxp\Model\DataObject\Data\Hotspotimage();
   $advancedImage->setImage($img);
   $items[] = $advancedImage;
}

$news->setGallery(new \OpenDxp\Model\DataObject\Data\ImageGallery($items));
...
```php
$hotspotImage = $object->getHotspot1(); // name of the field in OpenDXP is "hotspot1" in this case (class definition)
