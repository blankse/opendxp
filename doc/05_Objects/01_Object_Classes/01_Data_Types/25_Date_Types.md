...
The `date` and `date & time` object fields are represented by a calender widget in the OpenDXP GUI. 
...
The `date` and `date & time` object fields can be configured to respect the user's timezone. If timezone support is enabled the OpenDXP admin UI will display the date and time in the user's timezone. Without timezone support the date and time will be shown exactly as it was entered independent of the user's timezone.
...
with the usual calendar widget in the OpenDXP GUI or programmatically via the OpenDXP API.
