...
OpenDXP provides two sorts of inheritance. While data inheritance allows the inheritance of object data along the tree
...
If data inheritance is enabled and an attribute of an object is empty, OpenDXP tries to get this attribute from a parent 
...
In the OpenDXP backend, inherited values are visualized as in the screen below: they are grey and a bit transparent, 
...
> wrong values even after saving the object again. OpenDXP will disable the dirty detection
...
OpenDXP data objects support inheritance, just as any PHP object does. In OpenDXP the class from which a specific data 
class inherits can be changed. By default, a data class inherits from `OpenDxp\Model\DataObject\Concrete`, but if required 
...
> what consequences it might have when the parent class is changed from `OpenDxp\Model\DataObject\Concrete` to something else. 

In order to maintain all OpenDXP functionalities, it has to be ensured that the special class used in the example 
above extends `OpenDxp\Model\DataObject\Concrete` and that its methods don't override and clash in unexpected ways 
with existing methods of `OpenDxp\Model\DataObject\Concrete` or any magic functions of `OpenDxp\Model\DataObject\Concrete`
...
## Overriding OpenDXP Models
In addition to parent classes, it is also possible to override OpenDXP object classes with custom classes and tell OpenDXP 
to use the custom classes instead of the generated object classes. This can be done by using [model overrides](../../../20_Extending_OpenDxp/03_Overriding_Models.md).
