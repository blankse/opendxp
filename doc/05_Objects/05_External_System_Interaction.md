...
OpenDXP data objects can be created, filled and listed programmatically in order to realize batch imports and exports 
with only very few lines of code.

Therefore the recommended way of interacting with external systems is using the PHP API of OpenDXP and create a 
interaction layer with your custom PHP code. This interaction layer can be within a [OpenDXP plugin](../20_Extending_OpenDxp/13_Bundle_Developers_Guide/README.md), a library component,
...
OpenDxp\Console how to integrate your custom CLI scripts to the [OpenDXP console](../19_Development_Tools_and_Details/11_Console_CLI.md).
...
For simple CSV exports, OpenDXP backend interface provides a CSV export functionality.
 
 
## Memory Issues
If you're using / creating very much objects you should call the OpenDXP garbage collector after several cycles to prevent memory issues
...
```php
\OpenDxp::collectGarbage(["myImportantKey", "myConfig"]);

// You can also add items to the static list of globally protected keys by passing them to
$longRunningHelper = \OpenDxp::getContainer()->get(\OpenDxp\Helper\LongRunningHelper::class);
$longRunningHelper->addOpenDxpRuntimeCacheProtectedItems(["myVeryImportantKey", "mySuperImportKey", "..."]);

// This list is maintained as long as the process exists. You can remove protected keys again by calling
$longRunningHelper->removeOpenDxpRuntimeCacheProtectedItems(["myVeryImportantKey", "mySuperImportKey", "..."]);
