...
Please have a look at [System Requirements](../../23_Installation_and_Upgrade/01_System_Requirements.md) and ensure your system is ready for OpenDXP.

## 2. Install OpenDXP & Dependencies

The easiest way to install OpenDXP is from your terminal using Composer.
...
##### 1. Skeleton Package (only for experienced OpenDXP developers)
```bash
...
Keep in mind that OpenDXP needs to be installed **outside** of the **document root**.
...
OpenDXP requires write access to the following directories (relative to your project root): `/var`, `/public/var` ([Details](../../23_Installation_and_Upgrade/03_System_Setup_and_Hosting/03_File_Permissions.md))
...
:::info
 ...
 OpenDxp allows a fully automated installation process. Read more here: [Advanced Installation Topics](../02_Advanced_Installation_Topics/README.md)
...
Maintenance tasks are handled with Symfony Messenger. The `opendxp:maintenance` command will add the maintenance
messages to the bus. However, `opendxp:maintenance` will not process the maintenance messages itself.
Independent workers are required to consume these messages from the queues and perform maintenance tasks.
In a typical OpenDXP installation, you should set up workers for these transports:

| # | Transport name                 | Purpose                                                                                          |
|---|--------------------------------|--------------------------------------------------------------------------------------------------|
| 1 | opendxp_core                   | Core background tasks such as converting videos, rendering Asset previews, etc.                  |
| 2 | opendxp_maintenance            | [Maintenance tasks](../../20_Extending_OpenDxp/16_Maintenance_Tasks.md)                          |
| 3 | opendxp_scheduled_tasks        | [Scheduled tasks](../../18_Tools_and_Features/03_Scheduling.md)                                  |
| 4 | opendxp_image_optimize         | Execute optimizers on Asset image thumbnails                                                     |
| 5 | opendxp_asset_update           | Process Asset metadata e.g. Documents page count, Video duration/dimensions, Image previews etc. |
| 6 | opendxp_search_backend_message | Update the index for OpenDXP backend search. Only if you install `SimpleBackendSearchBundle`     |
...
Consuming messages from the queues is performed with the `messenger:consume` command (e.g., `bin/console messenger:consume opendxp_core ...`).
...
```bash
bin/console messenger:consume opendxp_core opendxp_maintenance opendxp_scheduled_tasks opendxp_search_backend_message opendxp_asset_update
bin/console messenger:consume opendxp_image_optimize
```
...
*/5 * * * * /your/project/bin/console opendxp:maintenance
...
*/5 * * * * /your/project/bin/console messenger:consume opendxp_core opendxp_maintenance opendxp_scheduled_tasks opendxp_search_backend_message --time-limit=300
*/5 * * * * /your/project/bin/console messenger:consume opendxp_asset_update --time-limit=300
*/5 * * * * /your/project/bin/console messenger:consume opendxp_image_optimize --time-limit=300
```

Refer to [Symfony Messenger](../02_Advanced_Installation_Topics/01_Symfony_Messenger.md) for further information on working
with the messenger component in OpenDXP.
