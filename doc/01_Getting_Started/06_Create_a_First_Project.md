...
In this section, you will learn the basics of OpenDXP, required to start developing. 


# Creating CMS Pages with Documents

In the first part you'll learn the basics for creating CMS pages with OpenDXP Documents. 
...
{% block content %}
    <h1>{{ opendxp_input("headline", {"width": 540}) }}</h1>

    {% for i in opendxp_iterate_block(opendxp_block('contentblock')) %}
...
OpenDXP uses by default Symfony Twig engine, so you have the full power of Symfony templates with all Symfony functionalities available. In addition to that, there are some OpenDXP specific additions like the so called *editables*, which add editable parts (placeholders) to the layout and some custom templating helpers. 

For details concerning editables (like `opendxp_input`, `opendxp_block`, ...) see [Editables](../03_Documents/01_Editables/README.md). 
...
### Putting It All Together With OpenDXP Documents
Now we need to connect the action to a page in the OpenDXP backend, so that the page knows which action 
...
![Create page](../img/OpenDxp_Elements_check_homepage.png)
...
[within OpenDXP](../04_Assets/03_Working_with_Thumbnails/01_Image_Thumbnails.md) using custom configurations.
...
![Add product class](../img/OpenDxp_Elements_class_add.png)
...
**Important:** Every generated class in the OpenDXP admin panel has an associated PHP class 
...
Now we can use it to create objects in OpenDXP.
...
One way to upload a photo is using this button: ![Upload image to an object](../img/OpenDxp_Elements_upload_button.png) or just drag a file that you uploaded from the Assets section.

Click *Save & Publish* button. 

That's it. 

![Complete object](../img/OpenDxp_Elements_complete_object.png)
...
`{{ editmode }}` is a standard variable (it's always set), that checks if the view is called from the OpenDXP admin backend and gives you the 
 possibility to do different stuff in editmode and in the frontend. 

`{{ opendxp_relation("product") }}` is one of the possible editable placeholders. It can be used to make 1 to 1 relations. A cool 
alternative for that would be the [Renderlet](../03_Documents/01_Editables/28_Renderlet.md) editable.  
Click [here](../03_Documents/01_Editables/README.md) for a full list of available editables in OpenDXP.
...
Go to the product page. In my case, let's say `http://opendxp.local/tshirt` where *tshirt* is the identifier of the product (the name visible the documents tree).
...
{% block content %}
    ...
    <div class="product-info">
        {% if editmode %}
            {{ opendxp_relation("product") }}
        {% else %}
            {% set product = opendxp_relation("product").element %} 
            {% if product %} 
                <h2>{{ product.name }}</h2>
                <div class="content">
...
OpenDXP automatically renders optimized images for certain output channels (including high-res @2x versions). 
