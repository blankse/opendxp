...
There are several components in the OpenDXP backend UI which are configured differently for each project. These are
...
the system translations shipped with OpenDXP, so basically you can translate anything within the backend UI. 
...
```twig
{{ opendxp_select("select", {
	"store": [
		["option1", {{ "Option One"|trans({}, 'admin', opendxp_editmode_admin_language()) }}],
		["option2", {{ "Option Two"|trans({}, 'admin', opendxp_editmode_admin_language()) }}],
		["option3", {{ "Option Three"|trans({}, 'admin', opendxp_editmode_admin_language()) }}]
	]
}) }}
```

#### Adding your own admin languages (since v6.3.6)
OpenDXP comes with a set of translations which are managed by [POEditor](https://poeditor.com).
