...
OpenDXP provides an easy way for editors to edit commonly used translation terms across the application, which can be found 
...
Shared translations are stored in `translations_messages` database table and can be versioned using [Migrations](docs/Development_Documentation/Development_Tools_and_Details/Migrations.html).

## Translations case sensitivity

Translations are case sensitive by default. You can
reconfigure OpenDXP to handle website and admin translations as case insensitive, however as this implies a performance
...
Since OpenDXP uses Symfony Translator component in background, you can store and use the translations in ICU Message Format(supported by Symfony Translator) for Pluralization, Selection and more. [Read here](https://symfony.com/doc/current/translation.html#message-format)
This can be achieved in OpenDXP by selecting type "ICU Format" on translation UI and passing required parameter in view/Controller. 
e.g.
...
## OpenDXP backend functionalities
