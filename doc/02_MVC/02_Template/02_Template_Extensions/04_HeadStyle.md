...
```twig
{# adding scripts #}
{% do opendxp_head_style().appendStyle(styles, {'conditional': 'lt IE 11'}) %}
```
...
```twig
{# adding styles #}
{% do opendxp_head_style().appendStyle(styles) %}
```
...
{% do opendxp_head_style().offsetSetStyle(100, customStyles) %}

{# place at end: #}
{% do opendxp_head_style().appendStyle(finalStyles) %}

{# place at beginning #}
{% do opendxp_head_style().prependStyle(firstStyles) %}
```
...
```twig
{% do opendxp_head_style().captureStart() %}
    body {
        background-color: red
    }
{% do opendxp_head_style().captureEnd() %}
```
...
**Note:** The `captureStart()` and `captureEnd()` methods are deprecated as of OpenDXP 11.4.0. Use a combination of `set` and `opendxp_head_style` to achieve the same result.
...
{% endset %}
{% do opendxp_head_style().appendStyle(inlineStyle) %}

{# Example if you wish to specify additional attributes for the <style> tag #}
{% do opendxp_head_style().appendStyle(inlineStyle, {"media": "screen"}) %}
```