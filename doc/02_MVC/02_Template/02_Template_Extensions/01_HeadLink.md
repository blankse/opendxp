...
{% do opendxp_head_link().appendStylesheet('/styles/basic.css') %}
{% do opendxp_head_link({rel: 'icon', href: '/img/favicon.ico'}, 'PREPEND').prependStylesheet('/styles/moz.css', 'screen', true,  {id: 'my_stylesheet'}) %}
...
{% do opendxp_head_link().enableWebLinks() %}

{# set web link attributes passed to every item #}
{% do opendxp_head_link().setWebLinkAttributes({as: 'style'}) %}
...
{% do opendxp_head_link().appendStylesheet('/static/css/styles.css', 'screen', false, {webLink: {as: 'style'}}) %}

{# override the used method (default is preload()) #}
{% do opendxp_head_link().appendStylesheet('/static/css/styles.css', 'screen', false, {'webLink': {method: 'prefetch'}}) %}
```
...
(this setting is enabled by default from OpenDXP's core config):
