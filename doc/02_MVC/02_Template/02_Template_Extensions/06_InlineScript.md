...
and any method of that extension is available; however, use the `opendxp_inline_script()` method in place of `opendxp_head_script()`.

> Note: opendxp_inline_script() should be used when you wish to include scripts inline in the HTML body. Placing scripts at the end of 
...
Some JS libraries need to be included in the HTML head; use OpenDxp_head_script() for those scripts. 
