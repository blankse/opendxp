...
Knowing how routing works in OpenDXP/Symfony is essential to understand how an application can be implemented and how 
...
In addition to the default routing provided by Symfony (which is of course used as well), OpenDXP has some special
routing capabilities for documents, custom routes, multi-site support and redirects. 
 
## Routing in OpenDXP
In OpenDXP, there are several ways how controllers can be reached. These routes are processed in a specific priority 
order as described below. 

#### 1. System / Symfony Routes:
OpenDXP defines a few system routes that are required for OpenDXP base functionality like the admin user-interface and may also other routes provided by custom bundles. These routes are just standard Symfony routes and have the highest 
priority. 
...
#### 3. OpenDXP Documents and Pretty URLs:
The path of OpenDXP Documents also defines its public URL. In addition to the path, so called pretty URLs can be defined for
...
When your application has functionality where there is no OpenDXP Document necessary (e.g. product lists, detail pages, 
