...
One essential routing option in OpenDXP are the OpenDXP Document paths. Along with Pretty URLs they define the address
...
```twig
<a href="{{ opendxp_document(2) }}">Test-Link</a>

/* OR */

{% set document = opendxp_document(2) %}
...
```twig
<a href="{{ opendxp_url({'key': 'value'}) }}">Test-Link</a>
```
