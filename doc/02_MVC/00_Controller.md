...
OpenDXP controllers play the designated role in the MVC pattern. They bind the design patterns together and contain or delegate 
...
OpenDXP offers an abstract class (`OpenDxp\Controller\FrontendController`), which can be implemented by your controllers.
This abstract class adds some OpenDXP specific dispatching features - especially in combination with OpenDXP Documents,
multi-language support etc. 

The naming of the file and the class is just the same as in Symfony. 

## OpenDXP Specialities and Examples
...
The `DefaultController` comes with OpenDXP. When you create an empty page in OpenDXP it will call 
