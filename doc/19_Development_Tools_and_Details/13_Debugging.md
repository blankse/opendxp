...
In this chapter, a few insights, tips and tricks for debugging OpenDXP are shown. This should give you a
head start when developing with OpenDXP. 

## Debug Mode
In order to include some specific debugging tools (profiler, toolbar, ...), OpenDXP relies on environment 
...
## Dev Mode
The development mode enables some debugging features. This is useful if you're developing on the core of OpenDXP or when 
...
For proper debugging of OpenDXP backend UI activate the DEV-Mode in [system settings](../18_Tools_and_Features/25_System_Settings.md).
...
## HTTP Headers
OpenDxp might add following headers to its responses to provide additional debug information, especially concerning full
page cache: 

If response is delivered directly from full page cache: 
* `X-OpenDxp-Output-Cache-Tag:output_<SOME_HASH>`: Cache tag of delivered information.  
* `X-OpenDxp-Cache-Date:<CACHE_DATE>`: Date when information was stored to cache. 

If response could be delivered from full page cache, but is not: 
* `X-OpenDxp-Output-Cache-Disable-Reason:<SOME REASON>`: Describes reason why response is not delivered directly from full
...
One additional header to identify that response is sent by OpenDXP. 
* `X-Powered-By:opendxp`: Added to every request.
