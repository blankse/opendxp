...
> and must not start with `/admin` if OpenDXP version \<= 6.0.5 ! 

Add custom entry for `OpenDxpCoreBundle:PublicServices:customAdminEntryPoint` in your routing.yaml:  
```yaml
...
> If you don't want to name your route `my_custom_admin_entry_point` you can set the route name in the opendxp_admin configuration like this:
> ```yaml
> opendxp_admin:
