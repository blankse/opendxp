# Authenticate Against OpenDXP Objects

As Symfony's security component is quite complex, OpenDXP provides base implementations to facilitate integrating the security
configuration with users stored as OpenDXP objects.
...
use OpenDxp\Model\DataObject\User as BaseUser;
...
Next, we configure OpenDXP to use our overridden class:
...
opendxp:
    models:
        class_overrides:
            'OpenDxp\Model\DataObject\User': 'App\Model\DataObject\User'
```


## Loading users with a User Provider

A user provider is responsible for finding matching user objects for a given username. OpenDXP ships an `ObjectUserProvider`
...
[Loading Service Definitions](../../20_Extending_OpenDxp/13_Bundle_Developers_Guide/01_Loading_Service_Definitions.md)):

```yaml
# config/services.yaml
services:
    # The user provider loads users by Username.
    # OpenDXP provides a simple ObjectUserProvider which is able to load users from a specified class by a configured
...
which is responsible for calculating and verifying password hashes. As OpenDXP's `Password` field definition already provides
...
at runtime. OpenDXP adds this as additional layer of configuration which allows to specify a password hasher factory per user
...
# hasher per user. This service is configured in opendxp.security.password_hasher_factories to handle our user model.
services:
...
factory service instead in `opendxp.security.password_hasher_factories`. This is just an additional way of building password hashers - if 
...
This should get you started with a custom authentication system based on OpenDXP objects. For further information see:
