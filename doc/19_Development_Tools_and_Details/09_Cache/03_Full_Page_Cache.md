...
request. If there are X-OpenDxp-Cache-??? (marked orange below) headers in the response they the
...
opendxp:
    full_page_cache:
        enabled: true
        lifetime: 120
        exclude_cookie: 'opendxp_admin_sid'
...
| Disable Cookie | You can define an additional cookie-name which disables the cache. The cookie "opendxp_admin_sid" (used for the OpenDXP admin UI) ALWAYS disables the output-cache to make editor's life easier ;-) 
...
{
    #[AsEventListener(\OpenDxp\Event\FullPageCacheEvents::CACHE_RESPONSE)]
    public function determineFullPageCache(\OpenDxp\Event\Cache\FullPage\CacheResponseEvent $event): void
    {
...
Just add the parameter `?opendxp_outputfilters_disabled=true` to the URL.

### Disable the Full Page Cache with a Cookie and a Bookmarklet
Per default the disable-cookie configuration is set to `opendxp_admin_sid`.

That means that if you're logged into OpenDXP (have a session-id cookie) you will always get the
content live and not from the cache.

#### Bookmarklet
If you have the cookie `opendxp_admin_sid` in your system configuration you can use the following
...
* <a href="javascript:(function() {document.cookie='opendxp_admin_sid=disablethecachebaby'+(Math.floor(Math.random() * 147483648) + 2000)+';path=/;';})()">Disable OpenDXP Cache</a>

* <a href='javascript:void((function(){var a,b,c,e,f;f=0;a=document.cookie.split("; ");for(e=0;e<a.length&&a[e];e++){f++;for(b="."+location.host;b;b=b.replace(/^(?:%5C.|[^%5C.]+)/,"")){for(c=location.pathname;c;c=c.replace(/.$/,"")){document.cookie=(a[e]+"; domain="+b+"; path="+c+"; expires="+new Date((new Date()).getTime()-1e11).toGMTString());}}}alert("Expired "+f+" cookies");})())'>Enable OpenDXP Cache</a>
