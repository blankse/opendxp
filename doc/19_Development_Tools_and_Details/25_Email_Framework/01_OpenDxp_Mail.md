...
The `OpenDxp\Mail` Class extends the [`Symfony\Component\Mime\Email`](https://symfony.com/doc/current/mailer.html#email-addresses)
Class and adds some features for the usage with OpenDXP.

If email settings are configured in your `config/config.yaml` then on initializing
`OpenDxp\Mail` object, these settings applied automatically. It is required to configure [email settings](./README.md#general-information) prior to using OpenDxp\Mail.

The `OpenDxp\Mail` Class automatically takes care of the nasty stuff (embedding CSS,
