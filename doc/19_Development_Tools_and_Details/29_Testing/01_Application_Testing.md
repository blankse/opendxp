...
OpenDXP applications can be tested with any PHP testing solution, but this page demonstrates 2 viable approaches:
...
In addition to Codeception's general features, OpenDXP's Codeception modules provide a set of helpers to bootstrap a OpenDXP installation from an empty installation. The `opendxp` module is able to drop and re-create the database and addtional modules like the `ClassManager` provide helper code to create OpenDXP classes from JSON exports. As the DB initialization is configurable, you should be able to use the module as you need it (e.g. by bootstrapping your application yourself or by just running tests without any DB/data initialization logic. You can find examples how to use those modules by looking through [OpenDXP's test setup](https://github.com/open-dxp/opendxp/tree/11.x/tests).

## PHPUnit

As OpenDXP is a standard Symfony application, you can use Symfony's PHPUnit testing setup exactly as described in
[Symfony's Testing Documentation](https://symfony.com/doc/current/testing.html). All you need to do is to create a custom
bootstrap file to ensure the OpenDXP startup process has everything it needs. Start by adding Symfony's PHPUnit bridge
to your project:
...
This is everything you need to write simple unit tests which do not depend on OpenDXP's environment. Just run the tests
...
### Bootstrapping OpenDXP

If you want to write more advanced tests involving OpenDXP objects or Symfony's container - e.g. functional tests testing
controllers - you need to make sure OpenDXP is properly bootstrapped before tests are run. Alter the config file to point
...
<phpunit xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
    ...
    <php>
        ...
        <env name="OPENDXP_PROJECT_ROOT" value="." />
...
`WebTestCase` as base classes for tests involving the container, but OpenDXP expects the container to be set via `OpenDxp::setContainer()`
after bootstrapping. This is automatically done for you if you use `OpenDxp\Test\KernelTestCase` and `OpenDxp\Test\WebTestCase`
as base classes, otherwise you need to make sure to overwrite `createKernel` and set the container on the `OpenDxp` class.
...
which is defined as `OPENDXP_TEST_DB_DSN` environment variable by default (see [config/packages/test/config.yaml](https://github.com/open-dxp/opendxp/blob/1.x/.github/ci/files/config/packages/test/config.yaml#L19)).
...
in mind to make sure OpenDXP is properly bootstrapped before tests are run.


## Codeception

For OpenDXP's core tests, OpenDXP uses Codeception which wraps PHPUnit and adds a lot of nice features, especially for 
organizing tests and for adding helper code which can be used from tests. You can basically use the same setup as OpenDXP's
core by defining a custom test suite and by using OpenDXP's core helpers. The most important helper is `\OpenDxp\Tests\Support\Helper\OpenDxp`
which extends Codeception's [Symfony Module](https://codeception.com/docs/modules/Symfony) for functional testing and adds
logic to bootstrap OpenDXP and to drop/re-create the database and class directory to an empty installation to have every 
test suite start from a clean installation. 

<div class="alert alert-warning">
<strong>WARNING:</strong> OpenDXP's codeception setup is targeted for CI use, where database and data structures are created
...
OpenDXP already ships a `codeception.dist.yml` which is set up to run OpenDXP's core tests. You might want to change this
...
sure OpenDXP can be bootstrapped during tests. Adjust according to your needs.
...
define('OPENDXP_PROJECT_ROOT', realpath(__DIR__ . '/..'));
...
// add the core opendxp test library to the autoloader - this could also be done in composer.json's autoload-dev section
// but is done here for demonstration purpose
require_once OPENDXP_PROJECT_ROOT . '/vendor/open-dxp/opendxp/tests/Support/Util/Autoloader.php';

Autoloader::addNamespace('OpenDxp\Tests\Support', OPENDXP_PROJECT_ROOT . '/vendor/open-dxp/opendxp/tests/Support');
```

The `tests/unit.suite.yml` should be fine for a standard unit testing setup without dependencies, but we need to alter the
functional test suite to initialize a test database and to boot OpenDXP's kernel before running tests. Configure the 
suite to use the `\OpenDxp\Tests\Support\Helper\OpenDxp` helper:
...
modules:
    enabled:
        ...
        - \OpenDxp\Tests\Support\Helper\OpenDxp:
            # CAUTION: the following config means the test runner
            # will drop and re-create the OpenDxp DB and purge var/classes
...
And a functional test testing the empty index page. As the `OpenDxp` helper is based Codeception's `Symfony` module you 
