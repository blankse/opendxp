...
OpenDXP uses [Codeception](https://codeception.com/) for testing its core features. 

To execute core tests, OpenDXP provides a `docker-compose.yaml` file to create a setup for running the tests. 
...
- OpenDXP git repository cloned locally 


## Preparations
- Change to `tests\bin` directory of the OpenDXP repository
- Execute `./init-tests.sh`, this does:  
  - Running `docker compose up -d` and setting up all necessary services (like db and redis). OpenDXP source code files 
...
For Redis, the `OPENDXP_TEST_REDIS_DSN` option is mandatory. If not using the Redis provided by the `docker-compose.yaml`, set it 
...
| OPENDXP_TEST           | 1                | **important** this will switch several directories (like /var/classes)                                                          |
| OPENDXP_TEST_SKIP_DB   | 1                | Skips DB setup. This does not skip the db-related tests but it<br/>reduces the setup time for tests that don't need a database. |
| OPENDXP_TEST_REDIS_DSN | redis://localhost| **required for REDIS tests**                                                                                                    |
...
The following table lists all groups currently used by OpenDXP's core tests. If you extend the tests or write new
...
If you have the extend the data model then please have a look at [Model.php](https://github.com/open-dxp/opendxp/blob/1.x/tests/Support/Helper/Model.php).
There you will find all class definitions used for testing.

### Perform PHPStan Analysis

First, get a copy of this [sample configuration file](https://github.com/open-dxp/opendxp/blob/1.x/doc/Samples/phpstan.neon) and place it in your root directory.
