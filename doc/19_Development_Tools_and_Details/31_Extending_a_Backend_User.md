# Extending the OpenDXP User

OpenDXP does not allow to extend the user directly. Instead it allows to create a relation 
between a user and one or more OpenDXP objects. This can be used to add information to a user 
or to associate one or more objects directly with a system user. 

This article presents an example where the `member` object is associated with a OpenDXP user. 
The screenshots show how this can be achieved through the OpenDXP backend UI. In the bottom 
...
`\OpenDxp\Model\DataObject\ClassDefinition\Data\User`.
...
```php
$objects = \OpenDxp\Model\DataObject\Service::getObjectsReferencingUser($userId);
```
