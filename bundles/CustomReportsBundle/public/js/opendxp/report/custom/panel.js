...
opendxp.registerNS("opendxp.bundle.customreports.custom.panel");
/**
 * @private
 */
opendxp.bundle.customreports.custom.panel = Class.create({
    ...
    getTree: function () {
        if (!this.tree) {
            var store = Ext.create('Ext.data.TreeStore', {
                ...
                proxy: {
                    type: 'ajax',
                    url: Routing.generate('opendxp_bundle_customreports_customreport_tree'),
                    reader: {
                        type: 'json'
                    }
                },
                    ...
                    items: [
                        {
                            ...
                            disabled: !opendxp.settings['custom-reports-writeable']
                        }
                    ]
                }
            });
    ...
    getTreeNodeListeners: function () {
        var treeNodeListeners = {
            ...
            'beforeitemappend': function( thisNode, newChildNode, index, eOpts ) {
                ...
                newChildNode.data.iconCls = "opendxp_icon_sql"
            }
        };
    ...
    openConfig: function (id) {
        ...
        var existingPanel = Ext.getCmp("opendxp_sql_panel_" + id);
        if(existingPanel) {
        ...
        Ext.Ajax.request({
            ...
            success: function (response) {
                ...
                var fieldPanel = new opendxp.bundle.customreports.custom.item(data, this);
                opendxp.layout.refresh();
            }.bind(this)
        });
    },
    ...
    addFieldComplete: function (button, value, object) {
        ...
        if (button == "ok" && value.length > 2 && regresult == value) {
            ...
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_customreports_customreport_add'),
                method: 'POST',
                params: {
                    name: value
                },
    ...
    deleteField: function (tree, record) {
        ...
        Ext.Msg.confirm(t('delete'), sprintf(t('delete_message_advanced'), t('portlet_customreport'), Ext.util.Format.htmlEncode(decodedName)), function (btn) {
            if (btn == 'yes') {
                Ext.Ajax.request({
                    url: Routing.generate('opendxp_bundle_customreports_customreport_delete'),
                    method: 'DELETE',
                    params: {
                        name: record.data.id
                    }
    ...
    doCloneField: function (tree, record, button, value) {
        if (button == "ok") {
            Ext.Ajax.request({
                url: Routing.generate('opendxp_bundle_customreports_customreport_clone'),
                method: 'POST',
                params: {
                    name: record.data.id,
                    newName: value
