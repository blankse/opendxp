opendxp.registerNS("opendxp.bundle.tinymce.editor");
opendxp.bundle.tinymce.editor = Class.create({
    languageMapping: {
        fr: 'fr_FR',
        pt: 'pt_PT',
        sv: 'sv_SE',
    ...
    initialize: function () {
        if(!parent.opendxp.wysiwyg) {
            parent.opendxp.wysiwyg = {};
            parent.opendxp.wysiwyg.editors = [];
        }
        parent.opendxp.wysiwyg.editors.push('TinyMCE');
        document.addEventListener(parent.opendxp.events.initializeWysiwyg, this.initializeWysiwyg.bind(this));
        document.addEventListener(parent.opendxp.events.createWysiwyg, this.createWysiwyg.bind(this));
        document.addEventListener(parent.opendxp.events.onDropWysiwyg, this.onDropWysiwyg.bind(this));
        document.addEventListener(parent.opendxp.events.beforeDestroyWysiwyg, this.beforeDestroyWysiwyg.bind(this));
    },
    ...
    createWysiwyg: function (e) {
        ...
        const userLanguage = opendxp.globalmanager.get("user").language;
        ...
        if(language !== 'en') {
            language = {
                language_url: '/bundles/opendxptinymce/js/langs/' + language + '.js',
                language: language,
            };
        } else {
            language = {};
        ...
        let defaultConfig = {};
        if('' !== subSpace && opendxp[e.detail.context][subSpace]) {
            defaultConfig = opendxp[e.detail.context][subSpace].wysiwyg ? opendxp[e.detail.context][subSpace].wysiwyg.defaultEditorConfig : {};
        }
        ...
        const finalConfig = Object.assign({
            ...
            init_instance_callback: function (editor) {
                ...
                editor.on('input', function (eChange) {
                    checkCharCount();
                    document.dispatchEvent(new CustomEvent(opendxp.events.changeWysiwyg, {
                        detail: {
                            e: eChange,
        ...
        document.dispatchEvent(new CustomEvent(opendxp.events.createWysiwygConfig, {
            detail: {
                data: finalConfig,
                context: e.detail.context
            }
    ...
    onDropWysiwyg: function (e) {
        ...
        if (data.elementType === "asset") {
            if (data.type === "image" && textIsSelected === false) {
                ...
                // converted by the opendxp thumbnailing service so that they can be displayed in the editor
                let defaultWidth = 600;
                let additionalAttributes = {};

                if (typeof data.imageWidth != "undefined") {
                    const route = 'opendxp_admin_asset_getimagethumbnail';
                    const params = {
                        id: id,
                        width: defaultWidth,
                        aspectratio: true
                    ...
                    if (data.imageWidth < defaultWidth
                        && in_arrayi(opendxp.helpers.getFileExtension(data.text),
                            browserPossibleExtensions)) {
                        uri = data.path;
                        additionalAttributes = mergeObject(additionalAttributes, {opendxp_disable_thumbnail: true});
                    }
...
new opendxp.bundle.tinymce.editor();
