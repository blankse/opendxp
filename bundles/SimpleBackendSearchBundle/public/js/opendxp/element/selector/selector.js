...
opendxp.registerNS('opendxp.bundle.search.element.selector.selector');

/**
 * @private
 */
opendxp.bundle.search.element.selector.selector = Class.create({
    initialize: function (multiselect, callback, restrictions, config) {
        ...
        const restrictionDefaults = {
            ...
            subtype: {
                document: opendxp.globalmanager.get('document_search_types'),
                asset: opendxp.globalmanager.get('asset_search_types'),
                object: opendxp.globalmanager.get('object_search_types')
            },
            specific: {
        ...
        let title = t('search');
        let iconCls = 'opendxp_icon_search';
        if (this.restrictions.type && this.restrictions.type.length == 1) {
            title = t(this.restrictions.type[0] + '_search');
            iconCls = 'opendxp_icon_' + this.restrictions.type[0] + ' opendxp_icon_overlay_search'
        }

        if(this.config["asTab"]) {
            let myTabId = "opendxp_search_" + uniqid();
    ...
    searchDocuments: function () {
        ...
        this.current = new opendxp.bundle.search.element.selector.document(this);
    },
    ...
    searchAssets: function () {
        ...
        this.current = new opendxp.bundle.search.element.selector.asset(this);
    },
    ...
    searchObjects: function () {
        ...
        this.current = new opendxp.bundle.search.element.selector.object(this);
    },
