...
opendxp.registerNS('opendxp.bundle.search.element.selector.object');

/**
 * @private
 */
opendxp.bundle.search.element.selector.object = Class.create(opendxp.bundle.search.element.selector.abstract, {
    fieldObject: {},
    gridType: 'object',
    ...
    getForm: function () {
        ...
        let possibleRestrictions = opendxp.globalmanager.get('object_search_types');
        let filterStore = [];
        let selectedStore = [];
    ...
    changeClass: function () {
        ...
        if(selectedClass.indexOf(",") > 0) { // multiple classes because of a comma in the string
            ...
            const classStore = opendxp.globalmanager.get("object_types_store");
    ...
    initClassStore: function (selectedClass, response, save) {
        ...
        this.itemsPerPage = opendxp.helpers.grid.getDefaultPageSize(-1);
        const gridHelper = new opendxp.object.helpers.grid(selectedClass, fields, Routing.generate('opendxp_bundle_search_search_find'), null, true);
        ...
        var columns = [
            {text: t("type"), width: 40, sortable: true, dataIndex: 'subtype',
                renderer: function (value, metaData, record, rowIndex, colIndex, store) {
                    return '<div style="height: 16px;" class="opendxp_icon_' + value + '" name="'
    ...
    getGridPanel: function (columns, gridfilters, selectedClass, save) {
        this.pagingtoolbar = opendxp.helpers.grid.buildDefaultPagingToolbar(this.store,{pageSize: this.itemsPerPage});
    ...
    openColumnConfig: function(selectedClass, classId) {
        ...
        const dialog = new opendxp.object.helpers.gridConfigDialog(columnConfig,
...
opendxp.bundle.search.element.selector.object.addMethods(opendxp.element.helpers.gridColumnConfig);
